...
export const getProfile = catchAsync(async (req: Request, res: Response) => {
  ...
  let stats = await Stats.findOne({ user: user._id }).select(
    "totalSolved easySolved mediumSolved hardSolved contestPoints",
  );
  ...
  const userResponse = {
    ...
    contestPoints: stats.contestPoints || 0,
  };

  // Calculate Global Rank
  const rank =
    (await Stats.countDocuments({
  ...
  const recentProblems = recentSubmissions.map((sub) => ({
    ...
    difficulty:
      (sub.problem as any).difficulty.charAt(0).toUpperCase() +
      (sub.problem as any).difficulty.slice(1),
...
export const getStreak = catchAsync(async (req: Request, res: Response) => {
  const user = (req as any).user;
