...
export const submitToJudge0 = async (
  ...
  try {
    ...
    logger.error("Error submitting to Judge0 (Self-Hosted):", error);
    throw new Error(
      `Failed to submit code to Judge0 (Self-Hosted): ${error.message}`,
    );
  }
};
...
export const submitBatchToJudge0 = async (
  ...
  try {
    ...
    logger.error("Error submitting batch to Judge0 (Self-Hosted):");
    if (error.response) {
    ...
    throw new Error(
      `Failed to submit batch to Judge0 (Self-Hosted): ${error.message}`,
    );
  }
};
...
export const getJudge0Result = async (token: string): Promise<Judge0Result> => {
  try {
    ...
    logger.error("Error fetching Judge0 result (Self-Hosted):", error);
    throw new Error(
      `Failed to fetch result from Judge0 (Self-Hosted): ${error.message}`,
    );
  }
};
 ...
 */
export const getBatchJudge0Results = async (
  tokens: string[],
  ...
  try {
    ...
    const submissions =
      response.data.submissions || (response.data as unknown as Judge0Result[]);
    ...
    logger.error("Error fetching batch Judge0 results (Self-Hosted):", error);
    throw new Error(
      `Failed to fetch batch results from Judge0 (Self-Hosted): ${error.message}`,
    );
  }
};
